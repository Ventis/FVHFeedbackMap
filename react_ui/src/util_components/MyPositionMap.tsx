...
class MyPositionMap extends React.Component<MapProps, MapState> {
  state = {
  ...
  render() {
    const { onLocationSelected, extraLayers, t } = this.props;
  ...
  getInitialPosition() {
    ...
    const currentLatLng = currentPosition && [
      (currentPosition as Location).lat,
      (currentPosition as Location).lon,
    ];
    return (
      (useUrl && positionFromUrl) ||
      locationLatLng ||
      currentLatLng ||
  ...
  refreshMap() {
    ...
    if (!this.state.userMovedMap)
      this.leafletMap.setView(position, position[2] || zoom || 18);
...
export default withTranslation()(MyPositionMap);
