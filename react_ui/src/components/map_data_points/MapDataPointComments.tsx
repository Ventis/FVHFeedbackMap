...
class MapDataPointComments extends React.Component<MapDataPointCommentsProps> {
  state = initialState;
  static contextType = AppContext;

  render() {
    const { mapDataPoint, refreshNote, t } = this.props;
    ...
    return (
      <div className="m-2 ml-3">
        ...
        <ErrorAlert
          message={t("Commenting failed. Try again maybe?")}
          status={error}
        />
        <textarea
  ...
  submit = () => {
    ...
    sessionRequest(mapDataPointCommentsUrl, { method: "POST", data }).then(
      (response) => {
  ...
  onDelete = (commentId: number) => {
    ...
    sessionRequest(mapDataPointCommentUrl(commentId), {
      method: "DELETE",
    }).then((response) => {
      if (response.status >= 400) this.setState({ error: true });
      else {
...
export default withTranslation()(MapDataPointComments);
