...
export default class MapDataPointsMap extends React.Component<
  MapDataPointsMapProps,
  MapDataPointsMapState
> {
  ...
  getMapLayer() {
    ...
    const mapDataPoints =
      this.props.mapDataPoints || (this.context as any).mapDataPoints;
    ...
    filteredMapDataPoints.forEach((mapDataPoint: MapDataPoint) => {
      ...
      const style = {
        ...
        opacity:
          selectedNotes && selectedNotes.includes(mapDataPoint.id as number)
            ? 0.4
            : 0.05,
        weight: 20,
      ...
      const latLng = {
        lng: mapDataPoint.lon,
        lat: mapDataPoint.lat,
      } as LatLngLiteral;
      ...
      if (onNoteSelected)
        marker.on("click", () => onNoteSelected(mapDataPoint));
