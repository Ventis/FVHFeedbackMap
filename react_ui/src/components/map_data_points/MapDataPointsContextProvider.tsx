...
import {
  AppContext,
  MapDataPointsContextType,
  MapDataPoint,
  MapDataPointsContext,
...
export default class MapDataPointsContextProvider extends React.Component<
  MapDataPointsContextProps,
  MapDataPointsContextState
> {
  state = initialState;
  ...
  render() {
    ...
    const context: MapDataPointsContextType = {
      ...
      loadMapDataPoints: this.loadMapDataPoints,
    };
    return (
      <MapDataPointsContext.Provider value={context}>
        {children}
  ...
  loadMapDataPoints = () => {
    sessionRequest(mapDataPointsUrl + "?page_size=1000").then(
      (response: any) => {
  ...
  refreshNote = (note: MapDataPoint) => {
    return sessionRequest(mapDataPointUrl(note.id as number)).then(
      (response) => {
